#include "a_star.h"
vector<string> A_Star::shortestPath(std::string start, std::string end, DataProcessor& data) {
    ...
    // internet suggests that priority queue by default sorts pairs based on the first.
    priority_queue<std::pair<double, int>, vector<std::pair<double, int>>, greater<double>> open_queue;

    // open_set keeps track of which ids need to be checked. this circumvents updating the priority queue
    ...
    while (!open_set.empty()) {
        ...
        for (int n_id : data.getNeighbors(curr_id)) {
            double tentative_g_score = g_score[curr_id] + data.avg_adj_list[curr_id][n_id];
...
int A_Star::heuristic(int start_id, int end_id, DataProcessor& data) {
    ...
    if (data.avg_adj_list[start_id].count(end_id) == 0) {
        return 10;
    }
    return data.avg_adj_list[start_id][end_id];
}

vector<string> A_Star::backtrack(int start, int end, std::map<int, int>& came_from, DataProcessor& data) {
